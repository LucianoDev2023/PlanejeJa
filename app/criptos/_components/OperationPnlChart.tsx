...
export function OperationPnlChart({
  ...
  const onCrosshairMove = useCallback(
    (param: MouseEventParams) => {
      ...
      // Calcular ROI no ponto para o tooltip
      const roiAtPoint = invested > 0 && profitValue !== null ? (profitValue / invested) * 100 : null;
  ...
  return (
    <div className="from-white/6 relative w-full overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b to-white/[0.02] shadow-[0_20px_70px_rgba(0,0,0,0.65)] backdrop-blur-2xl">
      ...
      <div className="relative p-4 sm:p-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Chip>PnL vs Price</Chip>
              {lastUpdatedAt && (
                <Chip>Update: {formatInTimeZone(lastUpdatedAt, TIMEZONE, "HH:mm:ss")}</Chip>
              )}
            </div>
            <h3 className="text-xl font-black text-white tracking-tight">
              Análise de <span className="text-primary">{symbol}</span>
            </h3>
            <div className="text-xs text-slate-400 font-medium">
                Price: <span className="text-slate-100">${formatTokenPrice(lastPrice)}</span>
                <span className="mx-2 text-white/10">|</span>
                PnL: <span className={profitColor}>{formatCurrency(lastProfit)}</span>
                <span className="mx-2 text-white/10">|</span>
                ROI: <span className={roiColor}>{invested > 0 ? `${roiPct.toFixed(2)}%` : "-"}</span>
            </div>
          </div>
          ...
          <div className="flex gap-2 items-center overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
            ...
            <KpiPillSmall label="Patrimônio" value={formatCurrency(equityNow)} valueClass={equityColor} />
            <button 
              onClick={() => setShowEquityPanel(!showEquityPanel)}
              className="h-10 px-4 rounded-xl border border-white/10 bg-white/5 text-[11px] font-bold text-slate-300 hover:bg-white/10 hover:text-white transition-all whitespace-nowrap"
            >
              {showEquityPanel ? "Ocultar Equity" : "Ver Equity"}
            </button>
          </div>
        </div>
         {/* Toolbar Compacta */}
        ...
        {/* Tooltip Premium */}
        {hoverInfo && (
          <div className="pointer-events-none z-20 mt-3 w-full rounded-2xl border border-white/10 bg-[#020617]/90 p-3 text-[11px] text-slate-100 shadow-[0_10px_40px_rgba(0,0,0,0.8)] backdrop-blur-2xl sm:absolute sm:right-6 sm:top-6 sm:mt-0 sm:w-[240px] animate-in fade-in zoom-in duration-200">
            <div className="mb-2 flex items-center justify-between border-b border-white/5 pb-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{hoverInfo.timeLabel}</span>
              <span className={`h-2 w-2 rounded-full ${(hoverInfo.profit ?? 0) >= 0 ? "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.6)]" : "bg-rose-400 shadow-[0_0_8px_rgba(251,113,133,0.6)]"}`} />
            </div>
            ...
            <div className="space-y-2">
                ...
                <span
                    className={`font-bold ${
                    (hoverInfo.profit ?? 0) >= 0
                        ? "text-emerald-400"
                        : "text-rose-400"
                    }`}
                >
                    {hoverInfo.profit == null ? "-" : formatCurrency(hoverInfo.profit)}
                </span>
                </div>

                <div className="flex items-center justify-between">
                <span className="text-slate-400">ROI do Ponto</span>
                <span
                    className={`font-bold ${
                    (hoverInfo.roiPct ?? 0) >= 0
                        ? "text-emerald-400"
                        : "text-rose-400"
                    }`}
                >
                    {hoverInfo.roiPct == null ? "-" : `${hoverInfo.roiPct.toFixed(2)}%`}
                </span>
                </div>
            </div>
          </div>
        ...
        {profitExtrema && (
         ...
         {/* Equity mini panel */}
        {showEquityPanel && (
          <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_300px] animate-in slide-in-from-bottom-2 duration-300">
            <div className="rounded-3xl border border-white/10 bg-black/20 p-4 shadow-inner">
              <div className="mb-4 flex items-center justify-between">
                <div>
                    <p className="text-sm font-bold text-white uppercase tracking-tighter">Equity Explorer</p>
                    <p className="text-[10px] text-slate-500 font-medium">Investido vs Corrente</p>
                </div>
              </div>
              <div
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 flex flex-col justify-between">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium text-slate-400">Capital Investido</span>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-black ${equityColor}`}>
                    {formatCurrency(equityNow)}
                  </span>
                </div>

                <div className="flex items-center justify-between border-t border-white/5 pt-4">
                  <span className="text-xs font-medium text-slate-400">Lucro Acumulado</span>
              ...
              <div className="mt-6 rounded-2xl bg-primary/10 p-3">
                <p className="text-[10px] font-medium text-primary leading-relaxed opacity-80">
                  A linha tracejada representa o seu custo base (Investido). A linha sólida mostra a evolução do valor total (Hold + PnL).
                </p>
              </div>
            </div>
          </div>
